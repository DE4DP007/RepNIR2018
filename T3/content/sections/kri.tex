...
\begin{enumerate}
	...
	\item
	$\varphi - \Im_0$-измеримый скалярный случайный процесс, заданный на $[\sigma ,0[$, где $\sigma =\text{max}\left\{\tau_{ij}, i=0,...,m, j=0,\dots,m_i\right\}$;
...
\textbf{Определение 2.} Будем говорить, что уравнения (1) \textit{экспоненциально } $p$\textit{{}-устойчивым}
...
\eqref{eq-1}.Традиционна для этого по уравнению \eqref{kri-eq-1} с начальными условиями \eqref{kri-eq-1a}, \eqref{kri-eq-1b} строят систему следующего вида
...
\eqref{eq-1b}. Следовательно, экспоненциальная  $p${}-устойчивость уравнения \eqref{kri-eq-1} эквивалентна экспоненциальной
...
\begin{equation}\label{kri-eq-3}
	\begin{matrix} x_1'(t)=-qx_1(t)+x_2(t)(t\ge 0),\hfill\null
	\\\text{dx}_2(t)=\left[-(a_{00}(t)-(a_{10}(t)-q)q)x_1(t)-(a_{10}(t)-q)x_2(t)+\overset{m_0}{\underset{j=1}{\sum
	}}c_{\mathit{j0}}(t)x_1(h_{\mathit{j0}}(t))\right]\text{dt}+\hfill\null \\\overset
...
\begin{equation}\label{kri-eq-7}
	\begin{matrix}\overline{x_1}'(t)=-q\overline{x_1}(t)+\overline{x_2}(t)\text{   }(t\ge 0),\hfill\null
	\\d\overline{x_2}(t)=[-(a_{00}(t)-(a_{10}(t)-q)q)\overline{x_1}(t)-(a_{10}(t)-q)\overline{x_2}(t)+\hfill\null
	\\\overset{m_0}{\underset{j=1}{\sum }}c_{\mathit{j0}}(t)(\overline{x_1}(h_{\mathit{j0}}(t))+\overline{\varphi
	}(h_{\mathit{j0}}(t)))]\text{dt}+\hfill\null \\\overset m{\underset{i=1}{\sum
	}}[-(a_{0i}(t)-a_{1i}(t)q)\overline{x_1}(t)-a_{1i}(t)\overline{x_2}(t)+\hfill\null \\\overset{m_i}{\underset{j=1}{\sum
	}}c_{\text{ji}}(t)(\overline{x_1}(h_{\text{ji}}(t))+\overline{\varphi
	}(h_{\text{ji}}(t)))]\mathit{dB}_i(t)\text{  }(t\ge 0),\hfill\null \end{matrix}
\end{equation}
где  $\overline{x_i}$\ {}- неизвестный скалярный случайный процесс на
$\text{  }]-\infty ,+\infty [$ \textsubscript{\ }такой, что  $\overline{x_i}(t)=0$ \textit\text{subscript{\ }}при
$t<0$ \ и  $\overline{\varphi }(t)$\ {}- известный скалярный случайный процесс на $\text{  }]-\infty ,+\infty
[$\textsubscript{ }\ такой, что  $\overline{\varphi }(t)=\varphi (t)$ \ при  $t\in [\sigma ,0[$ \ и  $\overline{\varphi
}(t)=0$ \ когда  $t$ \textit\text{subscript{\ }}не принадлежит полуинтервалу  $[\sigma ,0[$. Обозначим через
$\overline x(t,b,\varphi )$ \textit\text{subscript{\ }}решение системы \eqref{kri-eq-7}, удовлетворяющее условию \eqref{kri-eq-3b}.Очевидно, что
...
$0<\lambda <\text{min}\{q,A_{10}-q\}$ \ для  $i=1,2$, то получится систему

\begin{equation}\label{kri-eq-8}
	\begin{matrix}y_{1'}(t)=(\lambda -q)y_1(t)+y_2(t)\text{   }(t\ge 0),\hfill\null
	\\\text{dy}_2(t)=[-(a_{00}(t)-(a_{10}(t)-q)q)y_1(t)+(\lambda -(a_{10}(t)-q))y_2(t)+\hfill\null
	...
	}}[-(a_{0i}(t)-a_{1i}(t)q)y_1(t)-a_{1i}(t)y_2(t)+\hfill\null \\+\overset{m_i}{\underset{j=1}{\sum
...
\begin{equation}\label{kri-eq-9}
	\begin{matrix}y_1(t)=\text{exp}\{-(q-\lambda )t\}b_1+\overset t{\underset 0{\int }}\text{exp}\{-(q-\lambda
	)(t-s)\}y_2(s)\text{ds}\text{   }(t\ge 0),\hfill\null
...
}}=\begin{matrix}\sup\\t<0\end{matrix}\left(E|\varphi (t)|^{2p}\right)^{1/2p}$ \ и неравенствами \eqref{kri-eq-2}-\eqref{eq-4}.
...
\end{equation*}
Из последнего уравнения системы \eqref{kri-eq-9} с учетом предыдущих обозначений и неравенств \eqref{kri-eq-2}-\eqref{eq-4} получим

\begin{equation}\label{kri-eq-10}
	\begin{matrix}\overline{y_2}\le \|b_2\|_{k_{2p}^1}+.\hfill\null \end{matrix}
\end{equation}

Так как

\begin{equation*}
\begin{matrix}\sup\\t\ge 0\end{matrix}\overset t{\underset 0{\int }}\text{exp}\left\{-\overset t{\underset s{\int
}}\eta (\zeta )\mathit{d\zeta }\right\}\text{ds}=\begin{matrix}\sup\\t\ge
...
\begin{equation*}
\begin{matrix}\begin{matrix}\sup\\t\ge 0\end{matrix}\left(\overset t{\underset 0{\int
}}\text{exp}\left\{-2\overset t{\underset s{\int }}\eta (\zeta )\mathit{d\zeta
}\right\}\text{ds}\right)^{1/2}=\\\begin{matrix}\sup\\t\ge 0\end{matrix}\left(\overset
...
\begin{equation}\label{kri-eq-11}
	\begin{matrix}\overline{y_2}\le \|b_2\|_{k_{2p}^1}+\gamma
	_0\overline{y_2}+\overset{m_0}{\underset{j=1}{\sum }}c_{\mathit{j0}}\text{exp}\{\text{$\lambda \tau
			$}_{\mathit{j0}}??\overline{y_1}??\frac ?{\overline{a_{10}}-q-\lambda }?+??\frac{c_p\overset m{\underset{i=1}{\sum
		}}\left[\gamma _i\overline{y_1}+A_{1i}\overline{y_2}+\overset{m_i}{\underset{j=1}{\sum
		}}c_{\text{ji}}\text{exp}\{\text{$\lambda \tau
				$}_{\text{ji}}\}\overline{y_1}\right]}{\sqrt{\overline{a_{10}}-q-\lambda }}.+M(\lambda
	)\overline{\overline{\varphi }},??\end{matrix}
\end{equation}
...
Очевидно также, что  $Z(0)=Z$. В силу условий теоремы матрица  $Z$ является  $M${}-матрицей, а тогда при достаточно
...
что существуют положительные числа  $\lambda =\lambda _0$,  $K=||Z(\lambda _0)^{-1}||\text{max}\{1,M(\lambda _0)\}$
...
\textbf{Следствие.} \ Если существует положительное число  $q$ \ такое, что для \ уравнения \eqref{kri-eq-1} имеет место
неравенство
...
\textit\text{subscript{\ }}будет  $M${}-матрицей, так как все диагональные миноры матрицы  $Z$
\textit\text{subscript{\ }}положительны.
