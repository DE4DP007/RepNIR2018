...
В \cite{sms-stn-1-SharIzVuz} было показано, что система полиномов $\{T_{r,k}(x)\}_{k=0}^\infty$ образует базис в \linebreak $W^r_{L^2_\mu(-1,1)}$, кроме того, в \cite{sms-stn-1-SharIzVuz} показано, что ряд Фурье функции $f\in W^r_{L^2_\mu(-1,1)}$ имеет следующий вид
