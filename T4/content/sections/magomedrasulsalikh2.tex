...
Функции Хаара определяются следующим образом \cite{mmgmsr2-Kashin}:
...
%			x-\frac{k-1}{2^j}, & x \in I^{(j+1)}_{2k-1} \\ %\frac{i-1}{2^k}\le x\le \frac{2i-1}{2^{k+1}}\\
%			\frac{k}{2^{j}}-x, & x \in I^{(j+1)}_{2k} \\ %\frac{2i-1}{2^{k+1}}\le x\le \frac{i}{2^{k}}\\
t-2^{-n}(s-1), & x \in \Delta^{2s-1}_{n+1}, \\
2^{-n}s-t, & x \in \Delta^{2s}_{n+1}, \\
0, & x \in [0, 1)\backslash \Delta^{s}_{n}, %\text{$\frac{i}{2^{k}}\le x\le1$}.
\end{cases}
%		\quad 2^k < n \leq 2^{k + 1}, k \geq 0
\end{equation}
где $k = 2^n+s$, $n \in \mathbb{N}_0$, $s = \overline{1, 2^n}$.
%Мы ограничимся рассмотрением этой задачи для отрезка $[0,1]$.
...
Результат выполнения программы для первых 6 итераций приведен на рис. \ref{mmgmsr2-pic-ex-cos}.
